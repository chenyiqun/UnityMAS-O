...
def _compute_qwen3_5_hybrid_attn_params(config):
    ...
    # Qwen3.5 full attention q_proj also emits a sigmoid gate, so q_proj is 2x query size.
...
def _estimate_qwen3_5_flops(config, tokens_sum, batch_seqlens, delta_time, **kargs):
    ...
    if not all(hasattr(text_config, field) for field in required_hybrid_fields):
        return _estimate_qwen2_flops(config, tokens_sum, batch_seqlens, delta_time)
